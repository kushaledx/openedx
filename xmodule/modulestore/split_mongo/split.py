...
class SplitBulkWriteRecord(BulkOpsRecord):  # pylint: disable=missing-class-docstring
    def __init__(self):
        super().__init__()
...
class SplitBulkWriteMixin(BulkOperationsMixin):
    ...
    def _start_outermost_bulk_operation(self, bulk_write_record, course_key, ignore_case=False):  # pylint: disable=arguments-differ
        """
    ...
    def _end_outermost_bulk_operation(self, bulk_write_record, structure_key):  # pylint: disable=arguments-differ
        """
    ...
    def insert_course_index(self, course_key, index_entry):  # pylint: disable=missing-function-docstring
    ...
    def get_structure(self, course_key, version_guid):  # pylint: disable=missing-function-docstring
        bulk_write_record = self._get_bulk_ops_record(course_key)
        if bulk_write_record.active:
    ...
    def get_definitions(self, course_key, ids):
        ...
        if len(ids):  # pylint: disable=len-as-condition
...
class SplitMongoModuleStore(SplitBulkWriteMixin, ModuleStoreWriteBase):
    ...
    def get_courses(self, branch, **kwargs):  # pylint: disable=arguments-differ
        """
    ...
    def has_course(self, course_id, ignore_case=False, **kwargs):
        ...
        return CourseLocator(course_index['org'], course_index['course'], course_index['run'], course_id.branch) if course_index else None  # pylint: disable=line-too-long

    def has_library(self, library_id, ignore_case=False, **kwargs):  # pylint: disable=unused-argument
        """
    ...
    def get_item(self, usage_key, depth=0, **kwargs):  # pylint: disable=arguments-differ
        ...
        with self.bulk_operations(usage_key.course_key):
            ...
            if len(items) == 0:  # pylint: disable=no-else-raise
                raise ItemNotFoundError(usage_key)
            elif len(items) > 1:
                log.debug(f"Found more than one item for '{usage_key}'")
            return items[0]

    def get_items(self, course_locator, settings=None, content=None, qualifiers=None, include_orphans=True, **kwargs):  # pylint: disable=arguments-differ
        """
        Returns:
    ...
    def get_parent_location(self, locator, **kwargs):  # pylint: disable=arguments-differ
        """
    ...
    def create_item(self, user_id, course_key, block_type, block_id=None, definition_locator=None, fields=None,  # pylint: disable=arguments-differ
        ...
        * If the locator has a org and course and run but its version_guid != current head, it raises VersionConflictError.  # pylint: disable=line-too-long
        ...
        with self.bulk_operations(course_key):
            ...
            if definition_locator is None or isinstance(definition_locator.definition_id, LocalId):
                ...
                definition_locator, _ = self.update_definition_from_data(course_key, definition_locator, new_def_data, user_id)  # pylint: disable=line-too-long
    ...
    def create_child(self, user_id, parent_usage_key, block_type, block_id=None, fields=None, asides=None, **kwargs):  # pylint: disable=arguments-differ
        """
    ...
    def create_course(  # pylint: disable=arguments-differ
    ...
    def _create_courselike(  # pylint: disable=too-many-statements
        ...
        if versions_dict is None or master_branch not in versions_dict:
            ...
            definition_id = self.create_definition_from_data(locator, definition_fields, root_category, user_id).definition_id  # pylint: disable=line-too-long
    ...
    def update_item(self, block, user_id, allow_not_found=False, force=False, **kwargs):  # pylint: disable=arguments-differ
        """
        ...
        Creates a new course version. If the block's location has a org and course and run, it moves the course head  # pylint: disable=line-too-long
    ...
    def _update_item_from_fields(self, user_id, course_key, block_key, partitioned_fields,    # pylint: disable=too-many-statements
        ...
        with self.bulk_operations(course_key):
            ...
            if partitioned_fields.get(Scope.children, {}):  # purposely not 'is not None'
                serialized_children = [BlockKey.from_usage_key(child) for child in partitioned_fields[Scope.children]['children']]  # pylint: disable=line-too-long
    ...
    def create_xblock(
        ...
        if parent_xblock is None:
            ...
            if fields is not None:
                for field_name in inheritance.InheritanceMixin.fields:  # pylint: disable=not-an-iterable
    ...
    def _persist_subdag(self, course_key, xblock, user_id, structure_blocks, new_id):  # pylint: disable=missing-function-docstring
        # persist the definition if persisted != passed
        ...
        if xblock.has_children:
            for child in xblock.children:
                if isinstance(child.block_id, LocalId):
                    ...
                    is_updated = self._persist_subdag(course_key, child_block, user_id, structure_blocks, new_id) or is_updated  # pylint: disable=line-too-long
        ...
        with self.bulk_operations(destination_course):
            ...
            for subtree_root in subtree_list:
                if BlockKey.from_usage_key(subtree_root) != source_structure['root']:
                    ...
                    if len(parents) and not parent_found:  # pylint: disable=len-as-condition
    ...
    def delete_item(self, usage_locator, user_id, force=False):  # pylint: disable=arguments-differ
        """
        ...
        Creates a new course version. If the block's location has a org, a course, and a run, it moves the course head  # pylint: disable=line-too-long
        pointer. If the version_guid of the block points to a non-head version and there's been an intervening
        change to this item, it raises a VersionConflictError unless force is True. In the force case, it forks
        the course but leaves the head pointer where it is (this change will not be in the course head).
        """
        ...
        for child in block_fields.get('children', []):
            try:
                if child in inherited_from:
                    raise Exception(f'Infinite loop detected when inheriting to {child}, having already inherited from {inherited_from}')  # pylint: disable=line-too-long
    ...
    def _find_course_assets(self, course_key):
        ...
        except (InsufficientSpecificationError, VersionConflictError) as err:  # pylint: disable=unused-variable  # noqa: F841
    ...
    def convert_references_to_keys(self, course_key, xblock_class, jsonfields, blocks):  # pylint: disable=unused-argument
        """
    ...
    def _find_local_root(self, element_to_find, possibility, tree):  # pylint: disable=missing-function-docstring
    ...
    def _get_asides_to_update_from_structure(self, structure, block_key, asides):
        ...
        if asides:
            ...
            result_list = []
            for i, aside in enumerate(block.asides):  # pylint: disable=unused-variable  # noqa: B007
